...
{
    public static class PreCompileCodeClass_OLD
    {
        ...
        public static void PrecompileCode_OLD()
        {
            ...
            {
                string[] individualDirectories = dir.Split("\\");
                string lastDirectory = individualDirectories[individualDirectories.Length - 1];

                if (lastDirectory.StartsWith(".")) continue;

                if (!precompDirs.Contains(lastDirectory))
                {
                ...
                string precompiledDirPath = Path.Combine(executingAssemblyPath, "Precompiled", lastDirectory);
                string currentDirPath = Path.Combine(pathOfPackages, lastDirectory);
